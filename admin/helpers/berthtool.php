...
require_once(JPATH_ADMINISTRATOR.DS.'components'.DS.'com_berthtool'.DS.'helpers'.DS.'query.php');

class BerthtoolHelper
{
	
	/*
	 * Submenu for Joomla 1.6
	 */
	public static function addSubmenu($vName = 'location')
	{
        
		JSubMenuHelper::addEntry(
			JText::_('Location'),
			'index.php?option=com_berthtool&view=location',
			($vName == 'location')
		);
	...
	public static function getExtensions() 
	{
		...
		$jv = new JVersion();
		$alt_libdir = ($jv->RELEASE < 1.6) ? JPATH_ADMINISTRATOR.DS.'components'.DS.'com_berthtool' : null;
		JLoader::import('joomla.utilities.xmlelement', $alt_libdir);
		
		$xml = simplexml_load_file(JPATH_ADMINISTRATOR.DS.'components'.DS.'com_berthtool'.DS.'elements'.DS.'extensions.xml', 'JXMLElement');		        
...
abstract class JHtmlBerthtool
{
	...
	public static function extensions($ext) 
	{
		...
		foreach ($extensions as $extension) {   
			...
			$option->value = 'com_berthtool.'.$extension->name;
			$options[] = $option;			
		}		
	...
	public static function categories($extension, $cat_id,$name="categories",$title="Select Category", $config = array('attributes'=>'class="inputbox"','filter.published' => array(0,1)))
	{
			...
			$query->from('#__berthtool_categories AS a');
