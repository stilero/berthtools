<?php
/**
 * @version		$Id: form.php 96 2011-08-11 06:59:32Z michel $
...
class JForm
{
	...
	public function bind($data)
	{
		...
		foreach ($data as $k => $v) {
			...
			if (is_scalar($v) || (is_array($v) && !JFormHelper::isAssociative($v))) {
		...
		foreach ($fields as $field) {
			...
			if ($group) {
				...
				//if ($input->exists($group.'.'.$name)) {
					$output->setValue($group.'.'.$name, $this->filterField($field, $input->getValue($group.'.'.$name, (string) $field['default'])));
				//}
			} else {
				// Filter the value if it exists.
				//if ($input->exists($name)) {
					$output->setValue($name, $this->filterField($field, $input->getValue($name, (string) $field['default'])));
				//}
			}
		}
	...
	public function loadFile($file, $reset = true, $xpath = false)
	{
		...
		$xml = JFormHelper::getXML($file, true);
